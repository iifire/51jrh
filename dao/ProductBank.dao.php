<?php
class ProductBankDAO extends AbstractDao {
    protected $m_tableName = 'my_product_bank';
    
    function __construct($isdebug = false) {
        parent::__construct($isdebug, 1);
    }
    public function getList($curpage=1,$pagesize=100,$order='') {
    ...
    public function sortByRateWeek($num=8) {
    	...
    	$sql .= " order by product_rate desc limit {$num}";
        G_LOG(__FILE__, __LINE__, LP_DEBUG, "SQL: " . $sql . "\n");
        $result = array();
        //echo $sql;
        $statement = $this->dao->db->query($sql);
