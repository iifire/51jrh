...
class CompanyDAO extends AbstractDao {
    protected $m_tableName = 'my_company';
    ...
    public function getList($categoryId=null,$curpage=1,$pagesize=20) {
        ...
        $where = ' WHERE 1 ';
        if ($categoryId) {
        	$where .= " and company_category={$categoryId} ";
        }
    ...
    public function getDetail($companyId) {
    	G_LOG(__FILE__, __LINE__, LP_DEBUG, "EXEC " . __METHOD__ . " \n");
        $sql = "SELECT * FROM {$this->m_tableName} WHERE company_id={$companyId} ";
        G_LOG(__FILE__, __LINE__, LP_DEBUG, "SQL: " . $sql . "\n");
        $result = array();
        $statement = $this->dao->db->query($sql);
        $result = $this->dao->db->fetch();
    ...
    public function getHot($num=10) {
    	...
    	$sql .= " and company_hot=1 order by company_position asc limit {$num}";
        G_LOG(__FILE__, __LINE__, LP_DEBUG, "SQL: " . $sql . "\n");
        $result = array();
        //echo $sql;
        $statement = $this->dao->db->query($sql);
