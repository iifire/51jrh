<?php
class ProductNetDAO extends AbstractDao {
    protected $m_tableName = 'my_product_net';
    
    function __construct($isdebug = false) {
        parent::__construct($isdebug, 1);
    }
    public function getList($curpage=1,$pagesize=100,$order=' product_popular asc ') {
        G_LOG(__FILE__, __LINE__, LP_DEBUG, "EXEC " . __METHOD__ . " \n");
        
        $sql = "SELECT * FROM {$this->m_tableName}";
        $where = ' WHERE 1 ';
        $sql .= $where;
        $sql .= " order by {$order} limit ". (int)(($curpage-1) * $pagesize) . ',' . (int)$pagesize;
