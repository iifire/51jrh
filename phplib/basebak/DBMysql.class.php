...
class DBMysql {
	...
	public function ExecUpdate($stmt) {
		$sql_type = self :: get_sql_type($stmt);
		if ($sql_type != SQLT_INSERT && $sql_type != SQLT_UPDATE && $sql_type != SQLT_DELETE && $sql_type != SQLT_REPLACE) {
			throw new OssException("invalid statement.\n");
		}
		$result = mysql_query($stmt, $this->m_instance);
		if (!$result) {
	...
	public function ExecTrans($statements) {
		...
		for ($i = 0; $i < count($statements); ++ $i) {
			...
			if ($sql_type != SQLT_INSERT && $sql_type != SQLT_UPDATE && $sql_type != SQLT_DELETE && $sql_type != SQLT_SET && $sql_type != SQLT_REPLACE) {
				throw new OssException("invalid statement.\n");
			}
		}
	...
	private function get_sql_type(& $sql) {
		...
		if (strcmp($sql_type, "select") == 0)
			...
			if (strcmp($sql_type, "update") == 0)
				return SQLT_UPDATE;
			else
				if (strcmp($sql_type, "insert") == 0)
					return SQLT_INSERT;
				else
					if (strcmp($sql_type, "delete") == 0)
						return SQLT_DELETE;
					else
						if (strcmp($sql_type, "replace") == 0)
							return SQLT_REPLACE;
						else
							if (strcmp($sql_type, "set") == 0)
								return SQLT_SET;
		return SQLT_NONE;
	}
