	...
	function getPageRangeByPageId($pageid = null) {
		...
		if (isset ($this->_pageData[$pageid]) || is_null($this->_itemData)) {
			if ($this->_expanded) {
				...
				$max_surplus = ($pageid >= ($this->_totalPages - $this->_delta)) ? ($pageid - ($this->_totalPages - $this->_delta)) : 0;
			} else {
		...
		if ($this->_totalPages > (2 * $this->_delta + 1)) {
			if ($this->_expanded) {
				...
				for ($i = $this->_currentPage - $this->_delta - $expansion_before; $expansion_before; $expansion_before--, $i++) {
					...
					$links .= $this->_renderLink(str_replace('%d', $i, $this->_altPage), $i) . $this->_spacesBefore . ($print_separator_flag ? $this->_separator . $this->_spacesAfter : '');
				}
			}
