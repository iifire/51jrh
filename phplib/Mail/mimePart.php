...
class Mail_mimePart {
	...
	function Mail_mimePart($body = '', $params = array ()) {
		if (!empty ($params['eol'])) {
			...
			if (defined('MAIL_MIMEPART_CRLF')) { // backward-copat.
		...
		if (!empty ($params['filename'])) {
			$headers['Content-Type'] .= ';' . $this->_eol;
			$headers['Content-Type'] .= $this->_buildHeaderParam('name', $params['filename'], !empty ($params['headers_charset']) ? $params['headers_charset'] : 'US-ASCII', !empty ($params['language']) ? $params['language'] : null, !empty ($params['name_encoding']) ? $params['name_encoding'] : null);
		}
		...
		if (!empty ($params['disposition'])) {
			...
			if (!empty ($params['filename'])) {
				...
				$headers['Content-Disposition'] .= $this->_buildHeaderParam('filename', $params['filename'], !empty ($params['headers_charset']) ? $params['headers_charset'] : 'US-ASCII', !empty ($params['language']) ? $params['language'] : null, !empty ($params['filename_encoding']) ? $params['filename_encoding'] : null);
			}
		}

		if (!empty ($params['description'])) {
			$headers['Content-Description'] = $this->encodeHeader('Content-Description', $params['description'], !empty ($params['headers_charset']) ? $params['headers_charset'] : 'US-ASCII', !empty ($params['name_encoding']) ? $params['name_encoding'] : 'quoted-printable', $this->_eol);
		}
	...
	function _encodePartToFile($fh, $boundary = null, $skip_head = false) {
		...
		if (count($this->_subparts)) {
			...
			if ($this->_body) {
				...
				if ($this->_body_file) {
					fwrite($fh, $f_eol);
					$res = $this->_getEncodedDataFromFile($this->_body_file, $this->_encoding, $fh);
					if (PEAR :: isError($res)) {
						return $res;
					}
				}
	...
	function _buildHeaderParam($name, $value, $charset = null, $language = null, $encoding = null, $maxLength = 75) {
		...
		// RFC2231:
		$encValue = preg_replace_callback('/([^\x21,\x23,\x24,\x26,\x2B,\x2D,\x2E,\x30-\x39,\x41-\x5A,\x5E-\x7E])/', array (
			$this,
	...
	function encodeHeader($name, $value, $charset = 'ISO-8859-1', $encoding = 'quoted-printable', $eol = "\r\n") {
		...
		if (!empty ($separator)) {
			...
			if (preg_match('#([\x80-\xFF]){1}#', $value)) {
				...
				$value = Mail_mimePart :: encodeHeaderValue($value, $charset, $encoding, strlen($name) + 2, $eol);
			} else
				if (strlen($name . ': ' . $value) > 78) {
	...
	function _explodeQuotedString($delimiter, $string) {
		...
		for ($q = $p = $i = 0; $i < $strlen; $i++) {
			if ($string[$i] == "\"" && (empty ($string[$i -1]) || $string[$i -1] != "\\")) {
				...
				if (!$q && preg_match("/$delimiter/", $string[$i])) {
	...
	function encodeQP($str) {
		...
		$str = preg_replace_callback($regexp, array (
			'Mail_mimePart',
			'_qpReplaceCallback'
		), $str);
		...
		if ($encoding == 'B') {
			...
			for ($i = 1; $i <= $length; $i++) {
				...
				if ($line_length + $chunk_len == $maxLength || $i == $length) {
					...
					if ($line_length + $chunk_len > $maxLength) {
						if ($result) {
							$result .= "\n";
						}
						if ($prev) {
			...
			for ($i = 0; $i <= $length; $i++) {
				...
				if ($char == ' ') {
					...
					$char = preg_replace_callback($regexp, array (
						'Mail_mimePart',
						'_qpReplaceCallback'
					), $char);
