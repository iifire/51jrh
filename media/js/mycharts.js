...
(function($){
	$.fn.commchart = function(options) {
		...
		if (_cht_map[opts.cht]=="pie") {
			...
			_myoptions = $.extend(_myoptions, _myoptionscomm);
		}
		if (opts.chxpl) {
		...
		$("#"+opts.chcn).highcharts(_myoptions);
		 ...
		 //展示表格
		if (opts.tacn && $('#'+opts.tacn).length>0 && _tard.length>0) {
			var tbstring = '<table cellspacing="0" cellpadding="0" id="'+opts.tacn+'_table">';
			//header
			tbstring += "<thead><tr>"
			for (var i=0;i<_tahd.length;i++) {
				tbstring += "<th>"+_tahd[i]+"</th>"
			}
			tbstring += "</tr></thead>"
			tbstring += "<tbody>"
			for (var i=0;i<_tard.length;i++) {
				var tmp_tacd = _tacd[i].split(',');
				tbstring += "<tr>"
				for (var j=0;j<tmp_tacd.length;j++) {
					tbstring += "<td>"+tmp_tacd[j]+"</td>"
				}
				tbstring += "</tr>"
			}
			for (var i=0;i<_taft.length;i++) {
				tbstring += "<td>"+_taft[i]+"</td>"
			}
			tbstring += "</tr></tbody>"
			tbstring += "</table>"
			$('#'+opts.tacn).append(tbstring);
			$('#'+opts.tacn+'_table').flexigrid();
		}	
	}
...
//theme
Highcharts.theme = {
   colors: ['#ec0000', '#8bbc21', '#0198ff', '#ff7170', '#fe0000', '#92d14f', '#00b04e', '#4f81bc', '#6AF9C4'],
   chart: {
      backgroundColor: '#FFF',
      borderWidth: 0,
...
var highchartsOptions = Highcharts.setOptions(Highcharts.theme);/*  |xGv00|d1ff0a516012fe21602277c85a18a0a7 */